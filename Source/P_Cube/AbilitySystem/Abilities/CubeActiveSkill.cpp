...
void UCubeActiveSkill::SpawnHitBox(const FName HitboxName, const FName DamageName, const FVector& HitboxLocation, const bool bOnlyPlayer, AActor* InstigatorPlayer, AActor* TargetActor)
{
	const bool bIsServer = GetAvatarActorFromActorInfo()->HasAuthority();
	if ( !bIsServer ) return;
